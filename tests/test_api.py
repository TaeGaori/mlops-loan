...
# 학습할 때 LabelEncoder로 변환했던 범주형(문자형) 컬럼 목록
...
def mock_model():
    ...
    되도록 지나ㅉ 대신 가짜(MackMock)를 끼워넣는다.
...
def test_get_rist_grade_boundaries(probability, expected_grade):
    """
...
def test_map_to_korean_keeps_unknown_keys():
    ...
    mapped = LoanModel._map_to_korean({'age': 35, 'unkown_field': 'x'})
    assert mapped['나이'] == 35
    assert mapped['unkown_field'] =='x'
...
def test_predict_batch_empty_list_returns_empty(mock_model):
    """빈 리스트를 넣으면 모델을 호춣하지 않고 빈 리스트를 반환한다"""
...
def test_health_when_model_loaded(client):
    """모델이 적재된 상태 - status 는 healty, model_loaded는 True"""
...
def test_health_when_model_not_loaded(client, mock_model):
    ...
    pipline을 None으로 바꾸면 서버쪽에도 그대로 반영된다.
    """
...
def test_predict_approved(client, mock_model):
    ...
    response = client.lpost('/predict', json=_make_valid_request())
...
def test_predict_request_id_is_unique(client):
    """같은 요청을 두 번 보내도 request_id는 매번 새로 발급되어야 한다."""
...
def test_predict_passes_features_in_training_order(client, mock_model):
    ...
    학습과 서빙의 컬럼 순서가 어긋나면 Training-Serving Skew가 생겨 에러 없이
