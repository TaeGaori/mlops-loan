...
from app.models import LoanModel
from app.schema import LoanRequest, LoanResponse, BatchLoanRequest, BatchLoanResponse, ModelInfoResponse, EnhancedLoanResponse
...
async def lifespan(app: FastAPI):
    """
    FastAPI 서버의 시작(startup)과 종료(stutdown) 생명 주기를 관리한다.
    ...
    # app.state --> FastAPI 앱 하나에 따린 공용 보관함 같은 객체
    #                   여기에 넣어둔 model은 이후 /health, /predice 같은 모든 요청에서 app.state.model로 꺼내 사용할 수 있다.
...
async def health_check():
    ...
    로드 밸런서(ALB)가 이 엔드포인트를 주기적으로 호출해 서버 상태를 확인
    ...
    # 파이프라인이없으면 웹 서버는 살아있어도 에측 기능은 사용할 수 없다.
...
async def predict(request: LoanRequest):
    ...
    FastAPI는 함수 호출 전에 JSON 요청 본문을 LoanRequest로 검증하고, 반환값도 EnhancedLoanResponse 형식에 맞는지 다시 확인한다
    """
    ...
    try:
        ...
        #   남기는 정보이므로 model.py가 아니라 main.py API 계층에서만든다.
        #   model.py의 predict()는 그대로 두고, 이 값만 API 응답에 얹는다.
        #   - request_id --> 매 요청마다 새로 발급되는 UUID. 로그에서 "이 요청 하나"를 정확히 찾아낼 수 있는 키가 된다.
        #   - timestamp --> UTC 기준 ISO 8601 문자열. timezone.utc를 명시해야 서버가 어느시간대에 있든 로그 시각 해석이 항상 같다.
