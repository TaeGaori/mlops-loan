...
app/models.py
---------------------------------------
학습된 대출 심사 모델의 로밍, 전처리, 추론을 캡술화한 모듈.
...
class LoanModel:
    ...
    학습 때 쓴 전처리(인코딩, 스케일링)dhk rtjqld Eo tmsms wjscjflrk djrmtskaus
    ...
    def __init__(self):
        ...
        self.threshold: float = 0.5     # 입계값 (확률을 가지고 승인/거절로 변환하는 기준값)
        ...
        # 예) 재학습된 새 모델을 배포했을 때 예측 로그에 버전을 함게 남기면
    ...
    def load(self, model_dir: str = 'models') -> None:
        ...
        Dokerfile의 WORKDIR이 달라지든 이 코드는 항상 이 파일(model.py) 기준으로 models폴더를 찾으므로
        실행 위치에 영향을 받지 않는다.
        """
    ...
    #   이 클래스와 관련은 있지만, 인스턴스 상태를 쓸 필요가 없는 함수를 안에 묶어두는 용도 
    ...
    def predict(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # 확률을 정책 임계값과 ㅣ교해 최종 승인 여부를 결정한다.
    ...
    def _get_risk_grade(probability: float) -> str:
        """
        승인 확률 구간을 사람이 해석하기 쉬운 A~D등급으로 변환한다.
        """
        if probability >= 0.75:
            return 'A'
    ...
    def predict_batch(self, data_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        # predict_proba를 n행짜리 dfd에 "한 버난 호출" --> [:, 1]로 승인(1) 확률 열만 꺼낸다.
